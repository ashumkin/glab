...
func Test_jobsView(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "no titler: even stage count, doubled horizontal space",
			jobs: []*ViewJob{
				{
					Name:       "stage1-job1-really-long",
					Stage:      "stage1",
		...
		{
			name: "no titler: odd stage count",
			jobs: []*ViewJob{
				{
					Name:       "stage1-job1-really-long",
					Stage:      "stage1",
		...
		{
			name: "no titler: even stage count",
			jobs: []*ViewJob{
				{
					Name:       "stage1-job1-really-long",
					Stage:      "stage1",
			...
			expected: []string{
				...
				"  └─────────────────┘  └─────────────────┘  └─────────────────┘  └─────────────────┘",
				"                                                                                    ",
				"  ╔✔ stage1-job1-re…╗  ┌──● stage2-job1──┐  ┌──■ stage3-job1──┐  ┌──Ø stage4-job1──┐",
				"  ║                 ║  │                 │  │                 │  │                »│",
				"  ║          01m 01s║┬┬│                 │┬┬│                 │┬─│                 │",
				"  ╚═════════════════╝││└─────────────────┘││└─────────────────┘│ └─────────────────┘",
				"                     ││                   ││                   │                    ",
...
func TestCIView(t *testing.T) {
	...
	tests := []testCase{
		...
		},
		{
			...
			setupMock: func(tc *gitlabtesting.TestClient) {
				tc.MockPipelines.EXPECT().
					GetLatestPipeline("OWNER/REPO", &gitlab.GetLatestPipelineOptions{Ref: gitlab.Ptr("test-branch")}, gomock.Any()).
					Return(&gitlab.Pipeline{
						ID:        9,
						Ref:       "test-branch",
						SHA:       "ec87bbc03675992e8048c01f497a18c796a18399",
						Status:    "created",
						WebURL:    "https://gitlab.com/OWNER/REPO/-/pipelines/9",
						CreatedAt: &createdAt,
					}, nil, nil)
			},
			expectedErr:    assert.NoErrorf,
