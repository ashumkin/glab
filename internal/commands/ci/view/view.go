...
func logPageName(name string, id int) string {
	return "logs-" + name + "-" + strconv.Itoa(id)
}
...
func (o *options) run(args []string) error {
	...
	for errLoop == nil {
		...
		errLoop = drawView(o, commit, projectID, p.User.Name, client, pipelineID, pipelineCreatedAt)
		if errLoop != nil {
			return errLoop
		}
		if !o.interactive {
...
func drawView(
	...
	app.SetInputCapture(inputCapture(app, root, navi, inputCh, forceUpdateCh, pipelines, appSt, o, client, projectID, pipelineID))
	go updateJobs(app, pipelines, jobsCh, forceUpdateCh, appSt, client, commit)
	go func() {
		defer recoverPanic(app)
		for {
			app.SetFocus(root)
			jobsView(app, jobsCh, inputCh, root, client, projectID, pipelineID, o.titler, appSt)
			app.Draw()
		}
	}()
