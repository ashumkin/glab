	...
	`),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if opts.choose {
				...
				lastPipeline, err = api.GetLatestPipeline(opts.ApiClient, opts.ProjectID, opts.RefName)
				if err != nil {
					return err
				}
				if lastPipeline == nil {
				...
				opts.createCommitFromPipeInfo(&gitlab.PipelineInfo{ID: lastPipeline.ID, WebURL: lastPipeline.WebURL})
			} else if opts.PipelineID == -1 {
				...
				if opts.Commit.LastPipeline == nil {
					return fmt.Errorf("Can't find pipeline for commit: %s", opts.Commit.ID)
				}
			}
...
func drawView(opts ViewOpts) error {
	...
	app.SetInputCapture(inputCapture(app, root, navi, inputCh, forceUpdateCh, &pipelines, &appSt, opts))
	go updateJobs(app, &pipelines, jobsCh, forceUpdateCh, &appSt, opts)
	go func() {
		defer recoverPanic(app)
		for {
			app.SetFocus(root)
			jobsView(app, jobsCh, inputCh, root, opts, &appSt)
			app.Draw()
		}
	}()
			...
			if appSt.curJob.Kind == Job && (appSt.curJob.Status == "created" || appSt.curJob.Status == "pending" || appSt.curJob.Status == "running") {
				...
				modal := tview.NewModal().
					...
					SetDoneFunc(func(buttonIndex int, buttonLabel string) {
						...
						app.ForceDraw()
						job, err := api.CancelPipelineJob(opts.ApiClient, opts.ProjectID, appSt.curJob.ID)
						if err != nil {
							app.Stop()
							log.Fatal(err)
						}
...
func curPipeline(pipelines []gitlab.PipelineInfo, opts ViewOpts) gitlab.PipelineInfo {
	if len(pipelines) == 0 {
...
func updateJobs(
	...
	for {
		...
		var err error
		pipeline := curPipeline(*pipelines, opts)
...
func linkJobs(screen tcell.Screen, appSt *appState) error {
	...
	for i, j := range appSt.jobs {
		if _, ok := appSt.boxes["jobs-"+j.Name]; !ok {
			return errors.Errorf("jobs-%s not found at index: %d", j.Name, i)
		}
	}
	var padding int
